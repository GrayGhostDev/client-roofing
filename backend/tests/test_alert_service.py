...
class TestAlertService:
    ...
    def test_trigger_lead_alert_success(
        self, alert_service, mock_redis, mock_notification, mock_realtime, mock_db, sample_lead_data
    ):
    ...
    def test_trigger_lead_alert_no_available_members(
        self, alert_service, mock_redis, mock_notification, mock_realtime, mock_db, sample_lead_data
    ):
    ...
    def test_mark_responded_success(
        self, alert_service, mock_redis, mock_notification, mock_realtime, mock_db
    ):
    ...
    def test_mark_responded_late_response(
        self, alert_service, mock_redis, mock_notification, mock_realtime, mock_db
    ):
        ...
        alert_data = {
            ...
            "created_at": (
                datetime.utcnow() - timedelta(seconds=150)
            ).isoformat(),  # 2.5 minutes ago
        }
        mock_redis.get.return_value = json.dumps(alert_data)
    ...
    def test_escalate_if_no_response_success(
        self, alert_service, mock_redis, mock_notification, mock_realtime, mock_db
    ):
        ...
        assert notification_args["channels"] == [
            "email",
            "sms",
            "push",
            "call",
    ...
    def test_escalate_already_responded(self, alert_service, mock_redis, mock_notification):
        ...
        # Setup
        alert_data = {"alert_id": "alert-123", "status": AlertStatus.RESPONDED.value}
        mock_redis.get.return_value = json.dumps(alert_data)
    ...
    def test_escalation_timer_integration(
        self, alert_service, mock_redis, mock_notification, mock_realtime, mock_db
    ):
...
class TestAlertConvenienceFunctions:
    ...
    def test_trigger_lead_alert_function(
        self, mock_redis, mock_notification, mock_realtime, mock_db
    ):
