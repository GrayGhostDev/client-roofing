...
class TestLeadAssignment:
    ...
    def test_assign_lead_already_assigned(self, client, auth_headers, mock_supabase_client):
        ...
        mock_supabase_client.table.return_value.select.return_value.eq.return_value.execute.return_value.data = [
            {"id": lead_id, "assigned_to": old_member_id, "status": "contacted"}
        ]

        # Mock successful reassignment
        mock_supabase_client.table.return_value.update.return_value.eq.return_value.execute.return_value.data = [
            {"id": lead_id, "assigned_to": new_member_id}
        ]

        response = client.post(
            f"/api/leads/{lead_id}/assign",
            json={"team_member_id": new_member_id, "force_reassign": True},
            headers=auth_headers,
        )
    ...
    def test_assign_lead_without_auth(self, client):
        ...
        response = client.post(
            f"/api/leads/{lead_id}/assign", json={"team_member_id": str(uuid4())}
        )
    ...
    @pytest.mark.integration
    def test_assign_lead_with_notification(
        self, client, auth_headers, mock_supabase_client, mock_pusher_client
    ):
        ...
        mock_supabase_client.table.return_value.select.return_value.eq.return_value.execute.return_value.data = [
            {"id": lead_id, "first_name": "John", "last_name": "Doe", "status": "new"}
        ]

        mock_supabase_client.table.return_value.update.return_value.eq.return_value.execute.return_value.data = [
            {"id": lead_id, "assigned_to": team_member_id}
        ]

        response = client.post(
            f"/api/leads/{lead_id}/assign",
            json={"team_member_id": team_member_id, "send_notification": True},
            headers=auth_headers,
        )
    ...
    def test_assign_lead_validation(self, client, auth_headers):
        ...
        response = client.post(f"/api/leads/{lead_id}/assign", json={}, headers=auth_headers)
