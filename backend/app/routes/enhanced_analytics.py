...
from app.models.analytics_sqlalchemy import (
    AnalyticsTimeframe, KPIRequest, RevenueAnalysisRequest,
    TeamPerformanceRequest, AnalyticsRequest
)
...
def get_enhanced_conversion_funnel():
    ...
    try:
        ...
        stages = {
            ...
            'appointment_scheduled': len([l for l in leads if l.get('status') == 'appointment_scheduled']),
            'inspection_completed': len([l for l in leads if l.get('status') == 'inspection_completed']),
        ...
        if include_sources:
            for source in set(l.get('source', 'unknown') for l in leads):
                ...
                source_breakdown[source] = {
                    ...
                    'conversion_rate': round((len(source_converted) / max(len(source_leads), 1)) * 100, 2)
                }
        ...
        if include_temperature:
            for temp in ['hot', 'warm', 'cold']:
                ...
                temperature_breakdown[temp] = {
                    ...
                    'conversion_rate': round((len(temp_converted) / max(len(temp_leads), 1)) * 100, 2)
                }
        ...
        if include_recommendations:
            if any(b['severity'] == 'high' for b in bottlenecks):
                recommendations.append("Critical bottlenecks identified - immediate process review needed")

            if source_breakdown:
                best_source = max(source_breakdown.items(), key=lambda x: x[1]['conversion_rate'])
                recommendations.append(f"Focus marketing budget on {best_source[0]} - highest conversion rate at {best_source[1]['conversion_rate']}%")
    ...
    try:
        ...
        if include_breakdown:
            ...
            for forecast_item in forecast.get('forecasts', []):
                quarter = f"Q{((datetime.strptime(forecast_item['date'], '%Y-%m').month - 1) // 3) + 1}"
...
def get_enhanced_team_performance():
    ...
    try:
        ...
        if include_recommendations and performances:
            ...
            slow_responders = [p for p in performances if p.get('leads', {}).get('avg_response_time', 0) > 15]
            if slow_responders:
                recommendations.append({
                    'type': 'process',
                    'priority': 'medium',
...
def get_marketing_roi_analysis():
    ...
    try:
        timeframe_str = request.args.get('timeframe', 'mtd')
        channels = request.args.get('channels', '').split(',') if request.args.get('channels') else None
    ...
    try:
        ...
        if export_format == 'json':
            ...
            return Response(
                output.getvalue(),
                mimetype='text/csv',
                headers={'Content-Disposition': f'attachment;filename=enhanced_analytics_{timeframe_str}.csv'}
            )
...
def subscribe_to_realtime_updates():
    ...
    try:
        ...
        subscription = {
            ...
            'pusher_app_key': pusher_service.app_key if hasattr(pusher_service, 'app_key') else 'mock_key',
        }
