...
def complete_gmb_auth():
    ...
    try:
        ...
        if success:
            return jsonify({
                'success': True,
                'message': 'GMB authentication completed'
            }), 200
        else:
            return jsonify({
                'success': False,
                'error': error or 'OAuth completion failed'
            }), 400
...
def fetch_reviews():
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                'success': False,
                'error': error or 'Failed to fetch reviews'
            }), 500
...
def fetch_platform_reviews(platform):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                'success': False,
                'error': error or f'Failed to fetch {platform} reviews'
            }), 500
...
def analyze_review(review_id):
    ...
    try:
        ...
        review = reviews_service.supabase.client.table('reviews').select('*').eq(
            'id', review_id
        ).single().execute()
...
def suggest_response(review_id):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                'success': False,
                'error': error or 'Failed to generate suggestion'
            }), 500
...
def post_response(review_id):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                'success': False,
                'error': error or 'Failed to post response'
            }), 400
...
def create_campaign():
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                'success': False,
                'error': error or 'Failed to create campaign'
            }), 500
...
def send_review_request():
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                'success': True,
                'message': f'Review request sent via {method}'
            }), 200
        else:
            return jsonify({
                'success': False,
                'error': error or 'Failed to send request'
            }), 500
...
def get_review_alerts():
    ...
    try:
        ...
        negative_reviews = reviews_service.supabase.client.table('reviews').select('*').lte(
            'rating', 3
        ).is_('reply', None).execute()
        ...
        old_reviews = reviews_service.supabase.client.table('reviews').select('*').is_(
            'reply', None
        ).lte('created_at', week_ago).execute()
...
def get_response_templates():
    ...
    """
    try:
        templates = reviews_service.supabase.client.table('response_templates').select('*').execute()
...
def create_response_template():
    ...
    try:
        ...
        result = reviews_service.supabase.client.table('response_templates').insert(template).execute()
