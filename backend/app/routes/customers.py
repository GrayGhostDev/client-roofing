...
from app.models.customer_sqlalchemy import Customer as CustomerModel, CustomerStatusEnum, CustomerSegmentEnum
from app.schemas.customer import (
    Customer, CustomerCreate, CustomerUpdate,
    CustomerStatus, CustomerSegment, CustomerResponse
)
from app.services.customer_service import customer_service
from app.services.notification import notification_service
from app.utils.auth import require_auth
from app.utils.validation import validate_request
from app.config import get_supabase_client, get_pusher_client
...
def get_customer(customer_id: str):
    ...
    try:
        ...
        if request.args.get('include_history') == 'true':
            interactions = supabase.from_('interactions') \
                .select('*') \
        ...
        insights = customer_service.get_customer_insights(
            customer,  # Already a dict from database
            interactions,
            projects
        )
...
def create_customer():
    ...
    try:
        ...
        notification_service.send_notification(
            ...
            data={'customer_id': customer['id'], 'name': f"{customer['first_name']} {customer['last_name']}"}
        )
    ...
    try:
        ...
        if 'lifetime_value' in data or 'project_count' in data:
            ...
            if new_segment != customer.get('segment'):
                # Update segment
                segment_update = supabase.from_('customers') \
...
def create_customer_interaction(customer_id: str):
    ...
    try:
        ...
        customer = supabase.from_('customers').select('interaction_count').eq('id', customer_id).single().execute()
        if customer.data:
...
def calculate_lifetime_value(customer_id: str):
    ...
    try:
        ...
        projects_result = supabase.from_('projects') \
    ...
    try:
        ...
        response = {
            ...
            'nps_breakdown': {
                ...
                'nps_score': (promoters - detractors) / total_customers * 100 if total_customers > 0 else 0
            }
        }
...
def export_customers():
    ...
    try:
        ...
        response.headers['Content-Disposition'] = f'attachment; filename=customers_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.csv'
