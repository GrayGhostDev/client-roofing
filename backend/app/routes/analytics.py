...
def get_trends():
    ...
    try:
        ...
        while current_date < end_date:
            ...
            if metric == 'leads':
                result = supabase.table('leads').select('id').gte('created_at', current_date.isoformat()).lt('created_at', next_date.isoformat()).execute()
                value = len(result.data) if result.data else 0
            elif metric == 'revenue':
                result = supabase.table('projects').select('actual_amount').eq('status', 'completed').gte('completed_at', current_date.isoformat()).lt('completed_at', next_date.isoformat()).execute()
                value = sum(float(p.get('actual_amount', 0)) for p in (result.data if result.data else []))
            elif metric == 'conversion':
                leads_result = supabase.table('leads').select('id').gte('created_at', current_date.isoformat()).lt('created_at', next_date.isoformat()).execute()
                converted_result = supabase.table('leads').select('id').eq('status', 'converted').gte('created_at', current_date.isoformat()).lt('created_at', next_date.isoformat()).execute()
                ...
                result = supabase.table('customers').select('id').gte('created_at', current_date.isoformat()).lt('created_at', next_date.isoformat()).execute()
                value = len(result.data) if result.data else 0
...
def get_team_performance():
    ...
    try:
        ...
        if team_member_id:
            ...
            team_result = supabase.table('team_members').select('id', 'name', 'role').eq('is_active', True).execute()
...
def create_custom_report():
    ...
    try:
        ...
        for metric in data['metrics']:
            if metric == 'leads':
                query = supabase.table('leads').select('*').gte('created_at', start_date.isoformat()).lte('created_at', end_date.isoformat())
                ...
                query = supabase.table('projects').select('*').gte('created_at', start_date.isoformat()).lte('created_at', end_date.isoformat())
    ...
    try:
        ...
        if export_format == 'json':
            ...
            return Response(
                output.getvalue(),
                mimetype='text/csv',
                headers={'Content-Disposition': f'attachment;filename=analytics_{timeframe_str}.csv'}
            )
