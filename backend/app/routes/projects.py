...
def get_project(project_id: str):
    ...
    try:
        ...
        if "history" in includes:
            ...
            history_result = (
                supabase.from_("project_history")
                .select("*")
...
def delete_project(project_id: str):
    ...
    try:
        ...
        if update_result.data:
            # Broadcast real-time event
            realtime_service.trigger_event(
                channel="projects", event="project-deleted", data={"project_id": project_id}
            )
...
def get_projects_overview():
    ...
    try:
        ...
        scheduled_projects = len(
            [p for p in projects if p["status"] == ProjectStatus.SCHEDULED.value]
        )
        ...
        if completed:
            completion_times = [
                (
                    datetime.fromisoformat(p["end_date"]) - datetime.fromisoformat(p["start_date"])
                ).days
                for p in completed
            ]
        ...
        for project in projects:
            if project.get("actual_value") or project.get("estimated_value"):
                ...
                costs = (
                    project.get("materials_cost", 0)
                    + project.get("labor_cost", 0)
                    + project.get("subcontractor_cost", 0)
                    + project.get("other_costs", 0)
                )
        ...
        for project_type in ProjectType:
            overview["projects_by_type"][project_type.value] = len(
                [p for p in projects if p.get("project_type") == project_type.value]
            )

        # Count by priority
        for priority in ProjectPriority:
            overview["projects_by_priority"][priority.value] = len(
                [p for p in projects if p.get("priority") == priority.value]
            )

        # Count delayed projects
        now = datetime.utcnow()
        overview["delayed_projects"] = len(
            [
                p
                for p in projects
                if p.get("end_date")
                and datetime.fromisoformat(p["end_date"]) < now
                and p["status"] != ProjectStatus.COMPLETED.value
            ]
        )
