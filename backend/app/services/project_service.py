...
class ProjectService:
    ...
    def create_project(self, project_data: ProjectCreate, created_by: str) -> Tuple[bool, Optional[Dict], Optional[str]]:
        """
        ...
        try:
            ...
            if project_dict.get('estimated_value') and project_dict.get('materials_cost'):
                ...
                project_dict['profit_margin'] = ((project_dict['estimated_value'] - total_cost) / project_dict['estimated_value']) * 100
    ...
    def update_project(self, project_id: str, update_data: ProjectUpdate, updated_by: str) -> Tuple[bool, Optional[Dict], Optional[str]]:
        """
        ...
        try:
            ...
            if 'materials_cost' in update_dict or 'labor_cost' in update_dict or 'actual_value' in update_dict:
                ...
                value = update_dict.get('actual_value') or update_dict.get('estimated_value', old_project.get('estimated_value', 0))
    ...
    def _handle_status_change(self, project: Dict, old_status: str, updated_by: str):
        ...
        try:
            ...
            if project['status'] == ProjectStatus.IN_PROGRESS.value and old_status != ProjectStatus.IN_PROGRESS.value:
    ...
    def get_project_timeline(self, project_id: str) -> Optional[Dict]:
        """
                ...
                if project.get('end_date'):
                    ...
                    if project['status'] == ProjectStatus.COMPLETED.value:
                        ...
                        if timeline['duration_days'] > 0:
                            timeline['progress_percentage'] = min(100, (elapsed / timeline['duration_days']) * 100)

                    # Check if delayed
                    if datetime.utcnow() > end and project['status'] != ProjectStatus.COMPLETED.value:
    ...
    def calculate_project_profitability(self, project_id: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            total_cost = materials_cost + labor_cost + subcontractor_cost + permit_cost + other_costs

            # Use actual value if completed, otherwise estimated
            revenue = project.get('actual_value') if project['status'] == ProjectStatus.COMPLETED.value else project.get('estimated_value', 0)
            ...
            profitability = {
                ...
                'cost_breakdown': {
                    'materials_percentage': round((materials_cost / total_cost * 100), 2) if total_cost > 0 else 0,
                    'labor_percentage': round((labor_cost / total_cost * 100), 2) if total_cost > 0 else 0,
                    'other_percentage': round(((subcontractor_cost + permit_cost + other_costs) / total_cost * 100), 2) if total_cost > 0 else 0
                },
                'profitability_status': 'excellent' if gross_margin > 40 else 'good' if gross_margin > 25 else 'fair' if gross_margin > 15 else 'poor'
            }
    ...
    def get_project_resources(self, project_id: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            team_result = client.from_('project_team').select('*').eq('project_id', project_id).execute()
            team_members = team_result.data if team_result.data else []

            # Get material requirements
            materials_result = client.from_('project_materials').select('*').eq('project_id', project_id).execute()
            materials = materials_result.data if materials_result.data else []

            # Get equipment requirements
            equipment_result = client.from_('project_equipment').select('*').eq('project_id', project_id).execute()
            ...
            resources = {
                ...
                'materials': {
                    ...
                    'ordered': len([m for m in materials if m.get('status') == 'ordered']) if materials else 0,
                    'received': len([m for m in materials if m.get('status') == 'received']) if materials else 0
                },
                'equipment': {
                    'list': equipment,
                    'rental_cost': sum(e.get('rental_cost', 0) for e in equipment) if equipment else 0
                },
                'subcontractors': {
    ...
    def schedule_project(self, project_id: str, start_date: datetime, crew_id: Optional[str] = None) -> Tuple[bool, Optional[Dict], Optional[str]]:
        """
        ...
        try:
            ...
            update_result = client.from_('projects').update(update_data).eq('id', project_id).execute()
    ...
    def _check_schedule_conflicts(self, crew_id: str, start_date: datetime, end_date: datetime) -> Optional[str]:
        """
        ...
        try:
            ...
            result = client.from_('projects').select('*').eq('assigned_crew', crew_id).gte('end_date', start_date.isoformat()).lte('start_date', end_date.isoformat()).execute()

            if result.data:
                conflicts = [f"Project {p['name']} ({p['start_date']} to {p['end_date']})" for p in result.data]
    ...
    def get_project_documents(self, project_id: str) -> List[Dict]:
        ...
        try:
            ...
            result = client.from_('project_documents').select('*').eq('project_id', project_id).order('created_at', desc=True).execute()
    ...
    def add_project_document(self, project_id: str, document_data: Dict) -> Tuple[bool, Optional[Dict], Optional[str]]:
        """
