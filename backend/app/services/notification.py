...
from app.models.notification import (
    ...
    NotificationStatus, NotificationType, NotificationPreferences
)
...
class NotificationService:
    ...
    def send_notification(self,
        ...
        try:
            ...
            if notification:
                status = NotificationStatus.SENT if results['success'] else NotificationStatus.FAILED
    ...
    def _send_channel_notification(self,
                                  channel: str,
                                  type: Union[str, NotificationType],
    ...
    def _get_recipient_info(self,
        ...
        if recipient_id and self.supabase:
            try:
                result = self.supabase.from_('users').select('*').eq('id', recipient_id).single().execute()
                if result.data:
    ...
    def _get_user_preferences(self, user_id: Optional[str]) -> Optional[NotificationPreferences]:
        """Get user notification preferences."""
        if not user_id or not self.supabase:
            return None

        try:
            result = self.supabase.from_('notification_preferences') \
                .select('*') \
                .eq('user_id', user_id) \
                .single() \
                .execute()
    ...
    def _is_quiet_hours(self, preferences: Optional[NotificationPreferences]) -> bool:
    ...
    def _schedule_notification(self,
                             ...
                             channels: List[str],
                             priority: str,
                             send_at: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        result = self.supabase.from_('scheduled_notifications').insert(scheduled_notification).execute()
    ...
    def _update_notification_status(self,
        ...
        try:
            ...
            self.supabase.from_('notifications').update(update_data).eq('id', notification_id).execute()
    ...
    def retry_failed_notification(self, notification_id: str) -> bool:
        ...
        try:
            # Get notification
            result = self.supabase.from_('notifications') \
                .select('*') \
            ...
            self.supabase.from_('notifications').update({
                ...
                'next_retry_at': (datetime.utcnow() + timedelta(seconds=self.retry_delays[retry_count])).isoformat()
    ...
    def process_scheduled_notifications(self):
        ...
        try:
            ...
            result = self.supabase.from_('scheduled_notifications') \
                .select('*') \
            ...
            for scheduled in result.data:
                try:
                    ...
                    logger.error(f"Failed to process scheduled notification {scheduled['id']}: {str(e)}")
