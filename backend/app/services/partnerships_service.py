...
class PartnershipsService:
    ...
    def __init__(self):
        ...
        self.partner_categories = {
            ...
            "real_estate_agent": {
                ...
                "tier_multipliers": {"bronze": 1.0, "silver": 1.15, "gold": 1.25, "platinum": 1.35},
            },
            "property_manager": {
                "name": "Property Manager",
                "base_commission": 0.12,  # 12% base commission
                "tier_multipliers": {"bronze": 1.0, "silver": 1.1, "gold": 1.2, "platinum": 1.3},
            },
            "contractor": {
                "name": "General Contractor",
                "base_commission": 0.07,  # 7% base commission
                "tier_multipliers": {"bronze": 1.0, "silver": 1.15, "gold": 1.3, "platinum": 1.4},
            },
            "home_inspector": {
                "name": "Home Inspector",
                "base_commission": 0.09,  # 9% base commission
                "tier_multipliers": {"bronze": 1.0, "silver": 1.1, "gold": 1.2, "platinum": 1.35},
            },
        }
    ...
    def create_partner(self, partner_data: dict) -> tuple[bool, dict | None, str | None]:
        ...
        try:
            ...
            if result.data:
                ...
                return (
                    True,
                    {"partner_id": partner_id, "partner_code": partner_code, "api_key": api_key},
                    None,
                )
            else:
    ...
    def update_partner(self, partner_id: str, updates: dict) -> tuple[bool, str | None]:
    ...
    def get_partner(
        self, partner_id: str = None, partner_code: str = None, email: str = None
    ...
    def _calculate_partner_metrics(self, partner_id: str) -> dict:
        ...
        try:
            # Get all referrals
            referrals = (
                self.supabase.client.table("referrals")
                .select("*")
            ...
            conversion_rate = (
                (len(won_referrals) / total_referrals * 100) if total_referrals > 0 else 0
            )

            total_revenue = sum(float(r.get("project_value", 0)) for r in completed_referrals)
            total_commission = sum(
                float(r.get("commission_amount", 0)) for r in completed_referrals
            )

            avg_project_value = (
                (total_revenue / len(completed_referrals)) if completed_referrals else 0
            )
            ...
            pending_referrals = [
                r
                for r in referrals.data
                if r.get("status") in ["won", "completed"] and not r.get("commission_paid")
            ]
            pending_commission = sum(
                float(r.get("commission_amount", 0)) for r in pending_referrals
            )
    ...
    def create_referral(self, referral_data: dict) -> tuple[bool, dict | None, str | None]:
        ...
        try:
            ...
            partner = (
                self.supabase.client.table("partners")
                .select("*")
                .eq("id", referral_data["partner_id"])
                .single()
                .execute()
            )
            ...
            if result.data:
                ...
                self.supabase.client.table("referrals").update({"lead_id": lead_id}).eq(
                    "id", referral_id
                ).execute()
    ...
    def update_referral_status(
        self, referral_id: str, status: str, project_value: float = None
    ) -> tuple[bool, str | None]:
        ...
        try:
            ...
            partner = (
                self.supabase.client.table("partners")
                .select("*")
                .eq("id", referral.data["partner_id"])
                .single()
                .execute()
            )
            ...
            updates = {"status": status, f"{status}_at": datetime.utcnow().isoformat()}
            ...
            if status in ["won", "completed"] and project_value:
                ...
                commission = self._calculate_commission(
                    project_value, partner.data["category"], partner.data["tier"]
                )
            ...
            if result.data:
                # Send notification
                self.pusher.trigger(
                    f'partner-{referral.data["partner_id"]}',
                    "referral-updated",
                    {
                        "referral_id": referral_id,
                        "status": status,
        ...
        try:
            ...
            if partner.data and partner.data["tier"] != new_tier:
                self.supabase.client.table("partners").update(
                    {"tier": new_tier, "tier_updated_at": datetime.utcnow().isoformat()}
                ...
                self.pusher.trigger(
                    ...
                    "tier-updated",
                    {"new_tier": new_tier, "monthly_referrals": count},
                )
    ...
    def process_commission_payment(
        self, partner_id: str, referral_ids: list[str] = None
    ) -> tuple[bool, dict | None, str | None]:
    ...
    def mark_payment_complete(
        self, payment_id: str, transaction_id: str = None
    ) -> tuple[bool, str | None]:
        ...
        try:
            ...
            result = (
                self.supabase.client.table("commission_payments")
                .update(updates)
                .eq("id", payment_id)
                .execute()
            )
    ...
    def authenticate_partner(
        ...
        try:
            ...
            if not bcrypt.checkpw(
                password.encode("utf-8"), partner.data["password_hash"].encode("utf-8")
            ):
                return False, None, "Invalid credentials"
    ...
    def validate_partner_session(self, session_token: str) -> tuple[bool, dict | None, str | None]:
        ...
        try:
            session = (
                self.supabase.client.table("partner_sessions")
                .select("*")
    ...
    def get_partner_dashboard(self, partner_id: str) -> tuple[bool, dict | None, str | None]:
        ...
        try:
            ...
            materials = (
                self.supabase.client.table("marketing_materials")
                .select("*")
                .eq("active", True)
                .execute()
            )
    ...
    def _calculate_partner_trends(self, partner_id: str) -> dict:
        ...
        try:
            ...
            # Group by month
            monthly_data = defaultdict(lambda: {"referrals": 0, "revenue": 0, "commission": 0})
            ...
            for referral in referrals.data:
                ...
                if referral.get("status") == "completed":
                    ...
                    monthly_data[month_key]["commission"] += float(
                        referral.get("commission_amount", 0)
                    )
    ...
    def get_partnerships_analytics(self) -> tuple[bool, dict | None, str | None]:
        ...
        try:
            ...
            won_referrals = (
                len([r for r in referrals.data if r.get("status") == "won"])
                if referrals.data
                else 0
            )
            completed_referrals = (
                len([r for r in referrals.data if r.get("status") == "completed"])
                if referrals.data
                else 0
            )
            ...
            total_commission = (
                sum(
                    float(r.get("commission_amount", 0))
                    for r in referrals.data
                    if r.get("commission_amount")
                )
                if referrals.data
                else 0
            )
            pending_commission = (
                sum(
                    float(r.get("commission_amount", 0))
                    for r in referrals.data
                    if r.get("commission_amount") and not r.get("commission_paid")
                )
                if referrals.data
                else 0
            )
            ...
            category_stats = defaultdict(
                lambda: {"partners": 0, "referrals": 0, "revenue": 0, "commission": 0}
            )
            ...
            if referrals.data:
                for referral in referrals.data:
                    ...
                    partner = next(
                        (p for p in partners.data if p["id"] == referral.get("partner_id")), None
                    )
                    if partner:
                        category = partner.get("category")
                        if category:
                            ...
                            if referral.get("status") == "completed":
                                category_stats[category]["revenue"] += float(
                                    referral.get("project_value", 0)
                                )
                                category_stats[category]["commission"] += float(
                                    referral.get("commission_amount", 0)
                                )
            ...
            for partner in partners.data:
                partner_referrals = (
                    [r for r in referrals.data if r.get("partner_id") == partner["id"]]
                    if referrals.data
                    else []
                )
                    ...
                    partner_performance.append(
                        {
                            ...
                            "conversion_rate": len(
                                [
                                    ...
                                    if r.get("status") in ["won", "completed"]
                                ]
                            )
                            / len(partner_referrals)
                            * 100,
                            "total_revenue": sum(
                                float(r.get("project_value", 0))
                                for r in partner_referrals
            ...
            top_performers = sorted(
                partner_performance, key=lambda x: x["total_revenue"], reverse=True
            )[:10]
            ...
            analytics = {
                "summary": {
                    ...
                    "overall_conversion_rate": round(
                        (won_referrals / total_referrals * 100) if total_referrals > 0 else 0, 1
                    ),
                    "total_revenue": round(total_revenue, 2),
                    "total_commission": round(total_commission, 2),
                    "pending_commission": round(pending_commission, 2),
                    "average_project_value": round(
                        (total_revenue / completed_referrals) if completed_referrals > 0 else 0, 2
                    ),
                },
