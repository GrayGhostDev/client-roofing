...
class AppointmentsService:
    ...
    def __init__(self):
        ...
        self.google_redirect_uri = os.getenv('GOOGLE_REDIRECT_URI', 'http://localhost:5000/api/appointments/oauth2callback')
    ...
    def create_appointment(self,
                          ...
                          sync_to_calendar: bool = True) -> Tuple[bool, Optional[Dict], Optional[str]]:
        """
    ...
    def update_appointment(self,
        ...
        try:
            ...
            if 'scheduled_start' in updates:
                ...
                updates['scheduled_end'] = (new_start + timedelta(minutes=duration + self.buffer_time)).isoformat()
    ...
    def cancel_appointment(self,
                          appointment_id: str,
                          reason: str = None,
                          cancelled_by: str = None) -> Tuple[bool, Optional[str]]:
        """
    ...
    def check_availability(self,
                          ...
                          start_time: datetime,
                          duration: int,
                          exclude_appointment_id: str = None) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            if cached_slots:
                ...
                for slot in slots:
                    ...
                    if (start_time < slot_end and end_time > slot_start):
                        return False, f"Conflicts with existing appointment at {slot_start.strftime('%I:%M %p')}"
            ...
            bh_start = datetime.combine(
                start_time.date(),
                datetime.strptime(business_hours['start'], '%H:%M').time()
            )
            bh_end = datetime.combine(
                start_time.date(),
                datetime.strptime(business_hours['end'], '%H:%M').time()
            )

            end_time = start_time + timedelta(minutes=duration)

            if start_time < bh_start or end_time > bh_end:
                return False, f"Outside business hours ({business_hours['start']} - {business_hours['end']})"

            # Query database for conflicts
            query = self.supabase.table('appointments').select('*').eq('team_member_id', team_member_id).in_('status', [AppointmentStatus.SCHEDULED, AppointmentStatus.CONFIRMED])
            ...
            date_end = (start_time + timedelta(days=1)).replace(hour=0, minute=0, second=0).isoformat()
    ...
    def get_available_slots(self,
        ...
        try:
            ...
            result = self.supabase.table('appointments').select('scheduled_start', 'scheduled_end').eq('team_member_id', team_member_id).in_('status', [AppointmentStatus.SCHEDULED, AppointmentStatus.CONFIRMED]).gte('scheduled_start', date.replace(hour=0, minute=0).isoformat()).lt('scheduled_start', (date + timedelta(days=1)).replace(hour=0, minute=0).isoformat()).execute()
    ...
    def get_team_schedule(self,
        ...
        try:
            ...
            ).eq('team_member_id', team_member_id).gte('scheduled_start', start_date.isoformat()).lte('scheduled_start', end_date.isoformat()).order('scheduled_start').execute()
    ...
    def send_reminder(self, appointment_id: str, reminder_type: str = ReminderType.EMAIL) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            result = self.supabase.table('appointments').select(
                '*, customers!inner(name, email, phone), team_members!inner(name)'
    ...
    def complete_appointment(self,
                            appointment_id: str,
                            notes: str = None,
                            follow_up_required: bool = False) -> Tuple[bool, Optional[str]]:
        """
    ...
    def _sync_to_google_calendar(self, appointment: Dict) -> Optional[str]:
        ...
        try:
            ...
            # Get customer info
            customer_result = self.supabase.table('customers').select('name', 'email', 'phone').eq('id', appointment['customer_id']).execute()
        ...
        try:
            ...
            for minutes_before in self.reminder_times:
                ...
                if reminder_time > datetime.now():
                    ...
                    task_data = {
                        'appointment_id': appointment['id'],
                        'reminder_type': ReminderType.EMAIL if minutes_before >= 60 else ReminderType.SMS,
    ...
    def _update_availability_cache(self, team_member_id: str, date: datetime.date):
        ...
        try:
            ...
            result = self.supabase.table('appointments').select('id', 'scheduled_start', 'scheduled_end').eq('team_member_id', team_member_id).in_('status', [AppointmentStatus.SCHEDULED, AppointmentStatus.CONFIRMED]).gte('scheduled_start', start_of_day.isoformat()).lte('scheduled_start', end_of_day.isoformat()).execute()
