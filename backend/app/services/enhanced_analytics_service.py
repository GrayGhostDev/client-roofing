...
from app.models.analytics import (
    AnalyticsTimeframe, MetricCategory, ChartType, AlertLevel,
    KPIDefinition, MetricValue, ConversionFunnel, RevenueAnalytics,
    CustomerAnalytics, TeamPerformance, MarketingAnalytics,
    WeatherImpactAnalytics, BusinessAlert, DashboardConfig
)
...
class EnhancedAnalyticsService:
    ...
    def calculate_roofing_kpis(self, timeframe: AnalyticsTimeframe = AnalyticsTimeframe.MTD) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            insights = self._generate_business_insights(
                lead_metrics, revenue_metrics, operational_metrics, customer_metrics,
                seasonal_metrics, weather_metrics
            )
    ...
    def _calculate_enhanced_lead_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            leads_result = self.supabase.table('leads').select(
                ...
                'assigned_to', 'status', 'response_time_minutes', 'urgency',
                'property_value', 'roof_age', 'insurance_claim', 'zip_code'
            ...
            prev_leads = self.supabase.table('leads').select('id').gte(
            ...
            return {
                ...
                'quality': {
                    ...
                    'high_score_percentage': round((high_score_leads / max(total_leads, 1)) * 100, 2),
                ...
                'response_performance': {
                    ...
                    'benchmark_comparison': self._compare_to_benchmark(avg_response_time, self.benchmarks.avg_response_time_minutes, 'lower_better'),
                },
                ...
                'industry_context': {
                    'insurance_lead_percentage': round((insurance_leads / max(total_leads, 1)) * 100, 2),
                    'high_value_percentage': round((high_value_leads / max(total_leads, 1)) * 100, 2),
    ...
    def _calculate_enhanced_revenue_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            projects_result = self.supabase.table('projects').select(
                'id', 'created_at', 'quote_amount', 'final_amount', 'status',
                'project_type', 'roof_material', 'roof_size_sqft', 'is_insurance_claim',
                'completion_date', 'customer_id', 'city', 'state'
            ).gte('created_at', start_date.isoformat()).lte('created_at', end_date.isoformat()).execute()
            ...
            # Pipeline analysis
            active_projects = [p for p in projects if p.get('status') in ['scheduled', 'in_progress']]
            ...
            avg_insurance_value = (insurance_revenue / max(len(insurance_projects), 1)) if insurance_projects else 0
            avg_retail_value = (retail_revenue / max(len(retail_projects), 1)) if retail_projects else 0
            ...
            # Growth analysis
            prev_period_start = start_date - (end_date - start_date)
            prev_projects = self.supabase.table('projects').select(
                'final_amount'
            ).eq('status', 'completed').gte(
                'completion_date', prev_period_start.isoformat()
            ).lt('completion_date', start_date.isoformat()).execute()

            prev_revenue = sum(float(p.get('final_amount', 0)) for p in (prev_projects.data if prev_projects.data else []))
            revenue_growth = ((total_revenue - prev_revenue) / max(prev_revenue, 1)) * 100 if prev_revenue else 0

            # Project size analysis
            project_sizes = [float(p.get('final_amount', 0)) for p in completed_projects if p.get('final_amount')]
            ...
            return {
                ...
                'project_performance': {
                    ...
                    'benchmark_comparison': self._compare_to_benchmark(avg_deal_size, self.benchmarks.avg_project_value, 'higher_better'),
                },
                ...
                'insurance_vs_retail': {
                    ...
                    'insurance_percentage': round((len(insurance_projects) / max(len(completed_projects), 1)) * 100, 2),
                },
                'material_performance': material_performance,
                'size_distribution': size_distribution,
                'growth_metrics': {
                    'revenue_growth_percentage': round(revenue_growth, 2),
                    'growth_trend': 'increasing' if revenue_growth > 10 else 'decreasing' if revenue_growth < -10 else 'stable',
                },
                'forecasting': {
                    'monthly_run_rate': round(total_revenue / max((end_date - start_date).days / 30, 1), 2),
                    'projected_annual': round(total_revenue * (365 / max((end_date - start_date).days, 1)), 2),
                },
            }
    ...
    def _calculate_seasonal_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            for year_offset in range(1, 4):  # Last 3 years
                ...
                hist_leads = self.supabase.table('leads').select('id').gte(
                    'created_at', hist_start.isoformat()
                ).lte('created_at', hist_end.isoformat()).execute()

                hist_projects = self.supabase.table('projects').select('final_amount').eq(
                    'status', 'completed'
                ).gte('completion_date', hist_start.isoformat()).lte(
                    'completion_date', hist_end.isoformat()
                ).execute()

                hist_lead_count = len(hist_leads.data) if hist_leads.data else 0
                hist_revenue = sum(float(p.get('final_amount', 0)) for p in (hist_projects.data if hist_projects.data else []))
            ...
            current_leads = self.supabase.table('leads').select('id').gte(
                'created_at', start_date.isoformat()
            ).lte('created_at', end_date.isoformat()).execute()

            current_projects = self.supabase.table('projects').select('final_amount').eq(
                'status', 'completed'
            ).gte('completion_date', start_date.isoformat()).lte(
                'completion_date', end_date.isoformat()
            ).execute()

            current_lead_count = len(current_leads.data) if current_leads.data else 0
            current_revenue = sum(float(p.get('final_amount', 0)) for p in (current_projects.data if current_projects.data else []))

            # Calculate seasonal performance vs. historical
            seasonal_lead_performance = (current_lead_count / max(avg_seasonal_leads, 1)) * 100 if avg_seasonal_leads else 0
            seasonal_revenue_performance = (current_revenue / max(avg_seasonal_revenue, 1)) * 100 if avg_seasonal_revenue else 0
            ...
            return {
                ...
                'seasonality_insights': self._generate_seasonality_insights(season, seasonal_lead_performance, seasonal_revenue_performance),
            }
    ...
    def _calculate_weather_impact_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            while current_date <= end_date:
                day_leads = self.supabase.table('leads').select('id', 'urgency', 'insurance_claim').gte(
                    'created_at', current_date.isoformat()
                ).lt('created_at', (current_date + timedelta(days=1)).isoformat()).execute()

                lead_count = len(day_leads.data) if day_leads.data else 0
                emergency_leads = len([l for l in (day_leads.data or []) if l.get('urgency') == 'immediate'])
                insurance_leads = len([l for l in (day_leads.data or []) if l.get('insurance_claim')])
            ...
            return {
                ...
                'storm_impact_analysis': {
                    ...
                    'avg_storm_day_leads': statistics.mean([d['total_leads'] for d in potential_storm_days]) if potential_storm_days else 0,
                },
                'daily_breakdown': leads_by_day,
                'weather_recommendations': self._generate_weather_recommendations(emergency_percentage, insurance_percentage),
            }
    ...
    def _calculate_business_health_score(self, lead_metrics: Dict, revenue_metrics: Dict,
                                       operational_metrics: Dict, customer_metrics: Dict) -> float:
        ...
        try:
            ...
            response_time = lead_metrics.get('response_performance', {}).get('avg_response_time_minutes', 999)
            response_score = max(0, 100 - ((response_time - self.benchmarks.avg_response_time_minutes) * 2))
            components.append(response_score * 0.20)

            # Revenue growth component (20%)
            revenue_growth = revenue_metrics.get('growth_metrics', {}).get('revenue_growth_percentage', 0)
            growth_score = min(100, max(0, 50 + revenue_growth))  # 0% growth = 50 points, +50% = 100 points
            ...
            quote_to_close = revenue_metrics.get('project_performance', {}).get('quote_to_close_ratio', 0)
    ...
    def _generate_business_insights(self, lead_metrics: Dict, revenue_metrics: Dict,
                                  operational_metrics: Dict, customer_metrics: Dict,
                                  seasonal_metrics: Dict, weather_metrics: Dict) -> List[Dict]:
        """Generate actionable business insights"""
        insights = []

        try:
            # Lead response time insight
            avg_response = lead_metrics.get('response_performance', {}).get('avg_response_time_minutes', 0)
            ...
            # Seasonal insight
            seasonal_performance = seasonal_metrics.get('seasonal_comparison', {}).get('lead_performance_vs_historical', 0)
            ...
            emergency_percentage = weather_metrics.get('weather_indicators', {}).get('emergency_lead_percentage', 0)
            ...
            insurance_percentage = revenue_metrics.get('insurance_vs_retail', {}).get('insurance_percentage', 0)
    ...
    def _generate_intelligent_alerts(self, lead_metrics: Dict, revenue_metrics: Dict,
                                   operational_metrics: Dict) -> List[Dict]:
        ...
        try:
            ...
            while current_date < end_date:
                ...
                projects = self.supabase.table('projects').select(
                    'final_amount', 'completion_date'
                ).eq('status', 'completed').gte(
                    'completion_date', current_date.isoformat()
                ).lt('completion_date', month_end.isoformat()).execute()
            ...
            avg_monthly_revenue = statistics.mean(seasonal_factors.values()) if seasonal_factors.values() else 0
            ...
            if len(monthly_data) >= 12:
                ...
                accuracy_metrics = {
                    'mae': round(mae, 2),
                    'rmse': round(rmse, 2),
                    'mape': round(np.mean(np.abs((np.array(actuals) - np.array(predictions)) / np.maximum(np.array(actuals), 1))) * 100, 2),
                }
            else:
            ...
            result = {
                'forecasts': forecasts,
                'historical_data': [{'date': d['date'].strftime('%Y-%m'), 'revenue': d['revenue']} for d in monthly_data[-12:]],
    ...
    def _compare_to_benchmark(self, value: float, benchmark: float, direction: str) -> Dict[str, Any]:
        """Compare a value to industry benchmark"""
        if direction == 'higher_better':
            percentage = (value / max(benchmark, 1)) * 100
            status = 'excellent' if percentage >= 120 else 'good' if percentage >= 100 else 'below_target'
        else:  # lower_better
            percentage = (benchmark / max(value, 1)) * 100
            status = 'excellent' if percentage >= 120 else 'good' if percentage >= 100 else 'needs_improvement'
    ...
    def _generate_seasonality_insights(self, season: str, lead_performance: float, revenue_performance: float) -> List[str]:
        """Generate insights based on seasonal performance"""
        insights = []

        if season == 'spring' and lead_performance < 90:
            insights.append("Spring is peak roofing season - consider increasing marketing investment")
        ...
        if season == 'winter' and lead_performance > 80:
            insights.append("Above-average winter lead generation suggests emergency/insurance opportunities")

        return insights

    def _generate_weather_recommendations(self, emergency_percentage: float, insurance_percentage: float) -> List[str]:
        ...
        try:
            if self.pusher_service.is_available():
                self.pusher_service.trigger(
                    ...
                    {
                        'timestamp': datetime.utcnow().isoformat(),
                        'business_health_score': metrics.get('period_summary', {}).get('business_health_score'),
                        'key_metrics': {
                            'total_leads': metrics.get('leads', {}).get('totals', {}).get('total_leads', 0),
                            'total_revenue': metrics.get('revenue', {}).get('revenue_summary', {}).get('total_revenue', 0),
                            'conversion_rate': metrics.get('leads', {}).get('quality', {}).get('hot_lead_percentage', 0),
                        },
