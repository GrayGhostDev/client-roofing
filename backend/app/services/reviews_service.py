...
class ReviewsService:
    ...
    def complete_gmb_oauth(self, auth_code: str, redirect_uri: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            result = self.supabase.client.table('platform_credentials').upsert({
    ...
    def fetch_gmb_reviews(self, location_name: str = None) -> Tuple[bool, Optional[List[Dict]], Optional[str]]:
        """Fetch reviews from Google My Business"""
        try:
            # Load credentials if not in memory
            if not self.gmb_credentials:
                result = self.supabase.client.table('platform_credentials').select('*').eq(
                    'platform', 'google_my_business'
                ).single().execute()
    ...
    def fetch_yelp_reviews(self, business_alias: str = None) -> Tuple[bool, Optional[List[Dict]], Optional[str]]:
    ...
    def fetch_facebook_reviews(self) -> Tuple[bool, Optional[List[Dict]], Optional[str]]:
    ...
    def fetch_birdeye_reviews(self) -> Tuple[bool, Optional[List[Dict]], Optional[str]]:
        ...
        try:
            ...
            url = f'https://api.birdeye.com/resources/v1/review/businessId/{self.birdeye_business_id}'
    ...
    def fetch_all_reviews(self, refresh: bool = False) -> Tuple[bool, Optional[Dict], Optional[str]]:
        ...
        try:
            ...
            # Broadcast update
            self.pusher.trigger('reviews', 'reviews-updated', {
                'metrics': metrics,
                'new_reviews': len(all_reviews)
            })
    ...
    def generate_response_suggestion(self, review: Dict) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        try:
            ...
            if comment:
                ...
                if 'quality' in comment_lower and rating >= 4:
                    ...
                    template += " Our team takes pride in maintaining the highest professional standards."
    ...
    def post_review_response(self, review_id: str, response_text: str, platform: str) -> Tuple[bool, Optional[str]]:
    ...
    def _post_gmb_response(self, review_name: str, response_text: str) -> Tuple[bool, Optional[str]]:
    ...
    def _post_facebook_response(self, comment_id: str, response_text: str) -> Tuple[bool, Optional[str]]:
    ...
    def _post_birdeye_response(self, review_id: str, response_text: str) -> Tuple[bool, Optional[str]]:
    ...
    def create_review_campaign(self, customer_ids: List[str], campaign_name: str,
                             template_id: str = None) -> Tuple[bool, Optional[Dict], Optional[str]]:
        ...
        try:
            ...
            for customer_id in customer_ids:
                # Get customer info
                customer = self.supabase.client.table('customers').select('*').eq(
                    'id', customer_id
                ).single().execute()
    ...
    def send_review_request(self, customer_id: str, method: str = 'email') -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            review_links = {
                'google': f"https://g.page/r/CYourGooglePlaceId/review",
                'yelp': f"https://www.yelp.com/writeareview/biz/your-yelp-id",
                'facebook': f"https://www.facebook.com/pg/yourpage/reviews",
    ...
    def get_review_insights(self, days: int = 30) -> Tuple[bool, Optional[Dict], Optional[str]]:
        ...
        try:
            ...
            reviews = self.supabase.client.table('reviews').select('*').gte(
            ...
            insights = {
                ...
                'average_rating': statistics.mean([r.get('rating', 0) for r in reviews.data if r.get('rating')]),
                'response_rate': len([r for r in reviews.data if r.get('reply')]) / len(reviews.data) * 100,
    ...
    def _calculate_sentiment_trend(self, reviews: List[Dict]) -> str:
        ...
        try:
            ...
            weekly_avg = {week: statistics.mean(scores) for week, scores in weekly_sentiments.items()}
