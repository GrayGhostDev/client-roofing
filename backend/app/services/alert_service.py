...
class AlertService:
    ...
    def trigger_lead_alert(
        ...
        try:
            ...
            alert_data = {
                ...
                "response_deadline": (
                    datetime.utcnow() + timedelta(seconds=self.response_timeout)
                ).isoformat(),
            }

            # Store in Redis with TTL
            redis_client.setex(f"alert:{alert_id}", 3600, json.dumps(alert_data))  # 1 hour TTL
            ...
            realtime_service.trigger_event(
                ...
                event="lead-alert",
                data={"alert": alert_data, "action_required": "respond_immediately"},
            )
            ...
            # Log alert creation
            logger.info(
                f"Alert {alert_id} created for lead {lead_id}, assigned to {assigned_to['name']}"
            )
    ...
    def acknowledge_alert(self, alert_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            # Update in Redis
            redis_client.setex(f"alert:{alert_id}", 3600, json.dumps(alert_data))

            # Broadcast acknowledgment
            realtime_service.trigger_event(
                channel="team-alerts",
                event="alert-acknowledged",
                data={"alert_id": alert_id, "acknowledged_by": user_id},
            )
    ...
    def mark_responded(self, alert_id: str, user_id: str, response_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Log response time
            self.log_response_time(alert_data["lead_id"], user_id, response_seconds)
    ...
    def log_response_time(self, lead_id: str, responded_by: str, seconds: float) -> bool:
        ...
        try:
            ...
            redis_client.setex(member_key, 86400, json.dumps(stats))  # 24 hour TTL
    ...
    def get_team_response_metrics(
        self, team_id: str | None = None, period_days: int = 30
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "within_target_rate": (
                    (within_target / total_responses * 100) if total_responses > 0 else 0
                ),
    ...
    def _find_available_team_member(self, priority: AlertPriority) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            for member in team_members.data:
                ...
                scored_members.append({"id": member["id"], "name": member["name"], "score": score})
    ...
    def _get_alert_data(self, alert_id: str) -> dict[str, Any] | None:
    ...
    def _record_alert_in_database(
        self, alert_id: str, lead_id: str, assigned_to: str, priority: str
    ):
    ...
    def _get_team_members(self, team_id: str) -> list[dict[str, Any]]:
        ...
        try:
            supabase = get_supabase_client()
            members = (
                supabase.table("team_members").select("user_id").eq("team_id", team_id).execute()
            )
...
def trigger_lead_alert(lead_id: str, lead_data: dict[str, Any]) -> tuple[bool, str | None]:
...
def get_response_metrics(team_id: str | None = None, period_days: int = 30) -> dict[str, Any]:
