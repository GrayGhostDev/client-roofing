...
from app.models.interaction import (
    InteractionType, InteractionStatus, InteractionDirection,
...
class InteractionService:
    ...
    def create_interaction(self, interaction_data: InteractionCreate, created_by: str) -> Tuple[bool, Optional[Dict], Optional[str]]:
        """
        ...
        try:
            ...
            if result.data:
                ...
                self._update_customer_last_interaction(interaction.get('customer_id'), interaction['id'])
    ...
    def update_interaction(self, interaction_id: str, update_data: InteractionUpdate, updated_by: str) -> Tuple[bool, Optional[Dict], Optional[str]]:
        """
        ...
        try:
            ...
            if 'start_time' in update_dict or 'end_time' in update_dict:
                start = datetime.fromisoformat(update_dict.get('start_time', old_interaction.get('start_time')))
                end = datetime.fromisoformat(update_dict.get('end_time', old_interaction.get('end_time')))
                if start and end:
                    update_dict['duration_minutes'] = int((end - start).total_seconds() / 60)

            # Update interaction
            result = client.from_('interactions').update(update_dict).eq('id', interaction_id).execute()
    ...
    def _update_customer_last_interaction(self, customer_id: Optional[str], interaction_id: str):
        """
    ...
    def get_interaction_history(self, entity_id: str, entity_type: str = 'customer',
    ...
    def get_communication_summary(self, entity_id: str, entity_type: str = 'customer') -> Dict:
        ...
        try:
            ...
            for interaction in interactions:
                ...
                summary['interaction_types'][int_type] = summary['interaction_types'].get(int_type, 0) + 1
            ...
            if summary['interaction_types']:
                summary['preferred_channel'] = max(summary['interaction_types'],
                                                  key=summary['interaction_types'].get)
            ...
            summary['average_response_time_hours'] = sum(response_times) / len(response_times) if response_times else None
    ...
    def get_pending_follow_ups(self, assigned_to: Optional[str] = None,
        ...
        try:
            ...
            # Build query
            query = client.from_('interactions').select('*').eq('follow_up_required', True).eq('follow_up_completed', False)
            ...
            for follow_up in follow_ups:
                if follow_up.get('customer_id'):
                    customer_result = client.from_('customers').select('first_name, last_name, email, phone').eq('id', follow_up['customer_id']).execute()
                    if customer_result.data:
                        follow_up['customer'] = customer_result.data[0]
                elif follow_up.get('lead_id'):
                    lead_result = client.from_('leads').select('first_name, last_name, email, phone').eq('id', follow_up['lead_id']).execute()
                    if lead_result.data:
    ...
    def mark_follow_up_complete(self, interaction_id: str, notes: Optional[str] = None) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            result = client.from_('interactions').update(update_data).eq('id', interaction_id).execute()
    ...
    def get_interaction_analytics(self, start_date: datetime, end_date: datetime,
        ...
        try:
            ...
            # Build query
            query = client.from_('interactions').select('*').gte('interaction_time', start_date.isoformat()).lte('interaction_time', end_date.isoformat())
            ...
            for interaction in interactions:
                # Count by type
                int_type = interaction.get('interaction_type', 'unknown')
                analytics['interactions_by_type'][int_type] = analytics['interactions_by_type'].get(int_type, 0) + 1

                # Count by day
                int_time = datetime.fromisoformat(interaction['interaction_time'])
                day = int_time.strftime('%Y-%m-%d')
                analytics['interactions_by_day'][day] = analytics['interactions_by_day'].get(day, 0) + 1

                # Count by hour
                hour = int_time.hour
                analytics['interactions_by_hour'][hour] = analytics['interactions_by_hour'].get(hour, 0) + 1
            ...
            # Calculate averages
            analytics['average_duration_minutes'] = total_duration / len(interactions) if interactions else 0
            analytics['total_duration_hours'] = total_duration / 60

            # Calculate rates
            analytics['conversion_rate'] = (analytics['conversion_interactions'] / len(interactions) * 100) if interactions else 0
            analytics['follow_up_completion_rate'] = (follow_ups_completed / follow_ups_required * 100) if follow_ups_required > 0 else 100

            # Get top performers
            top_users = sorted(interactions_by_user.items(), key=lambda x: x[1]['count'], reverse=True)[:5]
            analytics['top_performers'] = [
                {
                    'user_id': user,
                    'interaction_count': data['count'],
                    'total_duration_minutes': data['duration'],
                    'average_duration_minutes': data['duration'] / data['count'] if data['count'] > 0 else 0
                }
    ...
    def auto_log_interaction(self, interaction_type: str, entity_id: str,
                            entity_type: str, details: Dict) -> Tuple[bool, Optional[Dict], Optional[str]]:
        """
    ...
    def get_interaction_templates(self, interaction_type: Optional[str] = None) -> List[Dict]:
        """
    ...
    def merge_duplicate_interactions(self, interaction_ids: List[str],
                                    primary_id: str) -> Tuple[bool, Optional[str]]:
        """
            ...
            for interaction in interactions:
                if interaction['id'] != primary_id:
                    if interaction.get('notes'):
                        all_notes.append(f"[Merged from {interaction['id']}]: {interaction['notes']}")
