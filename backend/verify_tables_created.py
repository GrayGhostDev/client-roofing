    ...
    try:
        ...
        for table in expected_tables:
            try:
                ...
                table_info[table] = {"rows": row_count, "accessible": True}
                print(f"✅ {table:15} - accessible ({row_count} rows)")

            except Exception as e:
                table_info[table] = {"rows": 0, "accessible": False, "error": str(e)}
        ...
        if len(accessible_tables) >= 8:
            ...
            try:
                # Test leads query
                leads = (
                    client.table("leads").select("first_name,last_name,status").limit(3).execute()
                )
                print(f"✅ Leads query successful ({len(leads.data)} records)")

                # Test customers query
                customers = (
                    client.table("customers").select("first_name,last_name").limit(2).execute()
                )
                ...
                print("2. Test API at: http://localhost:8001/api/leads")
